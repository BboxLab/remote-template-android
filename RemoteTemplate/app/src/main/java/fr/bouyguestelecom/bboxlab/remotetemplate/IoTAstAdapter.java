...
public class IoTAstAdapter extends ArrayAdapter<JSONArray>                                                   //Generic type's Array view
{
    private Context                 mCtx;                                                           //Current Activity data
    private List<JSONArray>         iotAstLst;                                                           //Generic Type List
    private LayoutInflater          lyIft;                                                          //XML parser to UI
    private Activity                atv;                                                            //Your current screen, duh !

    public IoTAstAdapter(Context ctx, List<JSONArray> iotAst, Activity atv)
    {
        super(ctx, 0, iotAst);                                                                       //Pre-construction attributes assignation
        mCtx = ctx;
        iotAstLst = iotAst;
        this.atv = atv;
    ...
    public View             getView(int pos, View convertView, ViewGroup parent)
    {
        ...
        final JSONArray         iotAst;                                    //Constant; Assign media from the current Media List's position

        iotAst = iotAstLst.get(pos);
